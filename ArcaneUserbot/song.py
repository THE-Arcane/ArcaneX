...
@Client.on_message(commandpro(["song", "music"]))
@errors
@sudo_users_only
async def song(client, message: Message):
    ...
    if not urlissed:
        ...
        return
    pablo = await client.send_message(message.chat.id, f"**🔎 sᴇᴀʀᴄʜɪɴɢ...** `{urlissed}`")
    ...
    capy = f"""
**♬♬️ 𝚂𝙾𝙽𝙶 𝙽𝙰𝙼𝙴:** [{thum}]({mo})
**♡ 𝑅𝐸𝑄𝑈𝐸𝑆𝑇𝐸𝐷 𝐵𝑌:** {message.from_user.mention}
"""
    ...
    await client.send_audio(
        ...
        progress_args=(
            pablo,
            c_time,
            f"**ᴅᴏᴡɴʟᴏᴀᴅɪɴɢ ғʀᴏᴍ ᴀʀᴄᴀɴᴇ sᴇʀᴠᴇʀ ⚡** `{urlissed}`",
            file_stark,
        ),
    )
    await pablo.delete()
...
@Client.on_message(commandpro(["/video", "vsong", "video"]))
@errors
@sudo_users_only
